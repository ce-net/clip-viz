...
// ---- end core ----
// clip-viz — the clip app explains itself.
defineVizCell({
  tag: "ce-viz-clip",
  app: "clip",
  kind: "fleet utility",
  what: "Ambient clipboard sync across your own fleet: copy on one machine, paste on another. No server, no account — sealed payloads ride the mesh's gossip layer between your paired devices only.",
  how: vizFlow(660, 130,
    vizBox(10, 40, 120, 50, "you copy", "any device") +
    vizArrow(130, 65, 190, 65) +
    vizBox(190, 40, 130, 50, "clip daemon", "seals payload") +
    vizArrow(320, 65, 420, 65, true, "gossip clip/sync/1") +
    vizBox(420, 40, 130, 50, "clip daemon", "unseals") +
    vizArrow(550, 65, 610, 65) +
    vizBox(610, 40, 40, 50, "you", "paste") +
    `<text class="lbl" x="330" y="118" text-anchor="middle">one install command puts it on every paired device: ce app install clip --on fleet=mine</text>`
  ),
  live: [
    { kind: "self", label: "this node" },
    { kind: "info", label: "transport", html: `gossip fan-out on <code>clip/sync/1</code> between paired devices — gossip has no ack by design; directed traffic is what carries must-arrive semantics` },
  ],
  foot: `<span>topic <code>clip/sync/1</code></span><a href="https://github.com/ce-net/clip">github.com/ce-net/clip</a>`,
});
